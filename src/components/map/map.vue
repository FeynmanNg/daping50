...
export default {
  ...
  data() {
    return {
      ...
      option: {
        ...
        animation: false, // 不能打开 todo
        geo: {
          show: true,
          map: 'china', // 要与 `registerMap()` 的第一个参数对应, 'china' 会显示南海诸岛
          roam: false, // 鼠标缩放+平移 不能打开 todo
          aspectScale: 0.9,
          zoom: 1.16,
  ...
  methods: {
    ...
    initMap() {
      ...
      const myChart = echarts.init(document.getElementById('baseMap'));
      ...
      // 点击事件
      myChart.on('click', (e) => {
        console.log('点击地图 e.data =', e.data)
        if (!e.data) {
        ...
        this.scatterData.forEach((item) => {
          ...
          item.label =
            item.name === e.name
              ? { backgroundColor: { image: scatterHBg } }
              : { backgroundColor: { image: scatterBg } };
          item.symbolSize = item.name === e.name ? 30 : 20;
          item.selected = item.name === e.name;
        });
        myChart.setOption(this.option, true);
      });
